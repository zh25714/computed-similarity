...
class Trainer(object):
    ...
    def train(self, dataset):
        ...
        for idx in tqdm(range(len(dataset)), desc='Training epoch ' + str(self.epoch + 1) + ''):
            ...
            linput_pad=sen_pad(linput)
            rinput_pad=sen_pad(rinput)
            linput_pad, rinput_pad = linput_pad.unsqueeze(0).to(self.device), rinput_pad.unsqueeze(0).to(self.device)
            #output = self.model(linput,rinput)
            target = target.to(self.device)
            output = self.model(ltree, linput, rtree, rinput,linput_pad,rinput_pad)
