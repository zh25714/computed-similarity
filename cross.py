...
def fold(train_dataset, k, test_fold_id):
    size = len(train_dataset)//k
    test = deepcopy(train_dataset)
    train = deepcopy(train_dataset)
    train.ltrees = train.ltrees[:test_fold_id*size]+train.ltrees[(test_fold_id+1)*size:]
    train.rtrees = train.rtrees[:test_fold_id*size]+train.rtrees[(test_fold_id+1)*size:]
    train.lsentences = train.lsentences[:test_fold_id*size]+train.lsentences[(test_fold_id+1)*size:]
    train.rsentences = train.rsentences[:test_fold_id*size]+train.rsentences[(test_fold_id+1)*size:]
    train.labels = torch.cat((train.labels[:test_fold_id*size],train.labels[(test_fold_id+1)*size:]))
    train.size = train.labels.size(0)
    test.ltrees = test.ltrees[test_fold_id*size:(test_fold_id+1)*size]
    test.rtrees = test.rtrees[test_fold_id * size:(test_fold_id + 1) * size]
    test.lsentences = test.lsentences[test_fold_id * size:(test_fold_id + 1) * size]
    test.rsentences = test.rsentences[test_fold_id * size:(test_fold_id + 1) * size]
    test.labels = test.labels[test_fold_id * size:(test_fold_id + 1) * size]
    test.size = test.labels.size(0)
